...
| Cases / Boundary-free monitoring | `cases/satellite-index/index.html`, `cases/satellite-index/demo/index.html`, `cases/satellite-index/demo/satellite-index.js`, `cases/satellite-index/three/index.html`, `cases/satellite-index/three/satellite-index-three.js`, `css/pages/satellite-index.css`, `tests/test-satellite-index-contract.js` |
...
| Boundary-free monitoring case framing changed | `cases/index.html`, `cases/satellite-index/index.html` — keep IA and back-links consistent; sub-nav is 01 Overview · 02 Explorer, demo/ URL preserved but unlinked |
...
| Cases / Boundary-free monitoring | `cases/index.html`, `cases/satellite-index/index.html`, `cases/satellite-index/demo/index.html`, `cases/satellite-index/demo/satellite-index.js`, `cases/satellite-index/three/index.html`, `cases/satellite-index/three/satellite-index-three.js`, `css/pages/satellite-index.css` |
...
- Boundary-free monitoring case IA: 01 Overview (`cases/satellite-index/index.html`), 02 Explorer (`cases/satellite-index/three/`). The Plotly demo (`cases/satellite-index/demo/`) is archived — URL preserved, all JS and HTML intact, but removed from sub-nav and cross-links on the overview and Explorer pages. The context link and H1 on the overview and Explorer read "Boundary-free monitoring". Do not add a Demo section back to sub-nav.
- Satellite Index live pipeline is connected: the WORKER_URL constants in `cases/satellite-index/demo/satellite-index.js` and `cases/satellite-index/three/satellite-index-three.js` point to the deployed Cloudflare Worker at https://satellite-worker.platoscave.workers.dev (not a secret, safe to commit). The Worker is a separate project outside this repo (satellite-worker). `runAnalysis()` fetches the combined analysis endpoint; `generateNdviGrid()` is the fallback on Worker, imagery, or decode failure. All Worker requests must include `X-API-Key: WORKER_API_KEY`.
- `MAPTILER_API_KEY` in `cases/satellite-index/demo/satellite-index.js` and `cases/satellite-index/three/satellite-index-three.js` is a client-side tile access key, domain-restricted to rndrssn.github.io and localhost. It is not a backend secret and is safe to commit. Do not treat it as a sensitive credential.
- `WORKER_API_KEY` in `cases/satellite-index/demo/satellite-index.js` and `cases/satellite-index/three/satellite-index-three.js` is a build-time placeholder `'__WORKER_API_KEY__'` in source. The GitHub Actions deploy workflow substitutes it with the real value from the `WORKER_API_KEY` GitHub Actions secret before uploading to Pages. Never replace the placeholder with a real value in source. To rotate: update the GitHub secret and redeploy; also run `wrangler secret put API_KEY --env production` in the satellite-worker repo.
...
- Explorer HUD colour scheme: the HUD uses a light paper background with dark ink tokens so the controls and metadata read on both the map and the rendered canvas. Control states use sage for inactive or untoggled state and gold for active or toggled state; do not use translucent white-on-dark styling.
