...
| Cases / Satellite Index | `cases/satellite-index/index.html`, `cases/satellite-index/demo/index.html`, `cases/satellite-index/demo/satellite-index.js`, `css/pages/satellite-index.css` |
...
- MapLibre GL JS and Plotly.js load from CDN (jsdelivr) in the Satellite Index demo. No vendored copies. This is an intentional PoC decision — do not vendor these unless the project graduates beyond PoC.
...
- Satellite Index Plotly scene has two surface traces: (1) a flat luminance base at z = -0.5 from the Worker's RGB PNG, percentile-stretched (2nd–98th percentile), opacity 0.7, warm neutral colorscale; (2) the NDVI surface above it. Trace order is base first, NDVI second.
