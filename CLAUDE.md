...
- Satellite Index live Worker calls are viewport-area guarded: `MAX_LIVE_VIEWPORT_AREA_KM2` is 1 km² / 100 hectares. Above that, the `Get NDVI for current map` button must be disabled and the /ndvi and /image endpoints must not be called.
- Satellite Index viewport readout is live: `updateViewportReadout()` shows area and selected grid size while the map moves, and switches to a zoom-in warning above 100 hectares.
- Satellite Index grid sizing is adaptive: `SMALL_VIEWPORT_GRID_SIZE` 512 at or below 10 hectares, `MEDIUM_VIEWPORT_GRID_SIZE` 256 from 10 to 100 hectares. This is a request/render sampling policy, not a claim of source resolution beyond Sentinel's native data.
...
- Satellite Index NDVI isolines are explicit base-plane `scatter3d` traces from `buildNdviContourTraces()`, not Plotly surface contours. Keep surface `contours` disabled so isolines do not appear on every NDVI height level. Isolines use rust `#8B3A2A`, interval 0.2 NDVI units, and `NDVI_CONTOUR_PLANE_Z`.
