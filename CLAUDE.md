...
- `MAPTILER_API_KEY` in `modules/satellite-index/demo/satellite-index.js` and `modules/satellite-index/three/satellite-index-three.js` is a client-side tile access key, domain-restricted to rndrssn.github.io and localhost. It is not a backend secret and is safe to commit. Do not treat it as a sensitive credential.
- `WORKER_API_KEY` in `modules/satellite-index/demo/satellite-index.js` and `modules/satellite-index/three/satellite-index-three.js` is a build-time placeholder `'__WORKER_API_KEY__'` in source. The GitHub Actions deploy workflow substitutes it with the real value from the `WORKER_API_KEY` GitHub Actions secret before uploading to Pages. Never replace the placeholder with a real value in source. To rotate: update the GitHub secret and redeploy; also run `wrangler secret put API_KEY --env production` in the satellite-worker repo.
