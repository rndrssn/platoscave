...
- Satellite Index demo (`cases/satellite-index/demo/`) uses synthetic fixture NDVI data generated by `generateNdviGrid()` in `cases/satellite-index/demo/satellite-index.js` — not live satellite data. The live pipeline (Cloudflare Worker → Sentinel Hub) is designed but not yet connected.
