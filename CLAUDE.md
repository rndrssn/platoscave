...
- Explorer base context control is a single binary `Terrain` switch inside the HUD toolbar. Default/off (`aria-pressed="false"`) means Basemap context using the cached MapTiler satellite texture; on (`aria-pressed="true"`) means Terrain context using MapTiler `contours-v2` vector tiles rendered through MapLibre into a paperlike contour texture and a Terrain-only explanatory note below the viewer. Do not reintroduce a visible Basemap peer button, and do not render shaded Terrain RGB imagery as the terrain view.
