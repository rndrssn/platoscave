...
- Explorer HUD structure: the satellite-three-hud overlay is positioned top-left inside the satellite-three-viewer and stays compact: controls row, transient status, and request area only. Scene metadata lives below the viewer and is hidden until analysis has run. The HUD is promoted to its own GPU compositing layer with transform translateZ(0) and will-change transform to prevent WebGL canvas compositing blur on the overlaid text. Do not move controls outside the viewer or change the GPU promotion.
...
- Explorer base context control is a two-option Basemap/Terrain switch styled like the CV/Skills switch. Basemap uses the cached MapTiler satellite texture. Terrain renders MapTiler `contours-v2` vector tiles through MapLibre into a clean contour texture; do not render shaded Terrain RGB imagery as the terrain view.
...
- Explorer HUD colour scheme: the HUD uses a translucent neutral grey background with dark ink tokens so the controls and metadata read on both the map and the rendered canvas. Control states use the Flow Explore bar neutral (`--ink-ghost`) for disabled or zoom-needed state and the CV switch yellow (`--experience-switch-surface`) for active or enabled state; do not use translucent white-on-dark styling.
