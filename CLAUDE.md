...
- `WORKER_API_KEY` in `modules/satellite-index/demo/satellite-index.js` and `modules/satellite-index/three/satellite-index-three.js` is a build-time placeholder `'__WORKER_API_KEY__'` in source. The Cloudflare Workers build (`scripts/cf-build.sh`) substitutes it with the real value from the `WORKER_API_KEY` build variable (Settings → Build → Build variables and secrets — build-time only, not accessible at runtime) at build time. Never replace the placeholder with a real value in source. To rotate: update the `WORKER_API_KEY` build variable and redeploy; also run `wrangler secret put API_KEY --env production` in the satellite-worker repo.
