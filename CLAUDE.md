...
- Satellite Index live Worker calls are viewport-area guarded: `MAX_LIVE_VIEWPORT_AREA_KM2` is 2 km² / 200 hectares. Above that, the `Get NDVI for current map` button must be disabled and the /ndvi and /image endpoints must not be called.
...
- Satellite Index controls hierarchy: `Get NDVI for current map` is the single primary action. The base-map toggle belongs to the Plotly surface chrome as a quiet `Base map` view setting with a compact switch; do not style it as a peer action button.
