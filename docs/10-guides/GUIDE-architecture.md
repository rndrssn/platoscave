...
- Deploy injection: `scripts/cf-build.sh` (the Cloudflare Workers build) substitutes the real key from the `WORKER_API_KEY` build variable (Settings → Build → Build variables and secrets) at build time
...
- Key rotation: update the `WORKER_API_KEY` Cloudflare Workers build variable and redeploy; also run `wrangler secret put API_KEY --env production` in the `satellite-worker` repo
...
Sentinel Hub credentials (`SENTINEL_CLIENT_ID` / `SENTINEL_CLIENT_SECRET`) are billable backend credentials that must stay server-side. The Worker enforces a frontend API key via `X-API-Key`, centralises CORS and request validation, and keeps the frontend a static site (Cloudflare Workers static assets).

### Request flow

```
Browser (Cloudflare Workers static assets: platoscave.bedrockrebel.app)
    │  POST /analysis
