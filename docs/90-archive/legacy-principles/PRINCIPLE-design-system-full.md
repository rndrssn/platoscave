...
Content is part of the interface, not decoration. Every module should use language that is:

- **Ontologically correct**: terms must match the model (for Garbage Can: problem, choice opportunity, deliberation, oversight, flight, resolved problem).
- **Taxonomically consistent**: the same concept uses the same label across narrative, taxonomy, explorer, assessment, simulation, and diagnosis.
- **Semantically precise**: do not imply causal claims the model does not make (for example, a closed choice opportunity does not always mean a resolved problem).
- **Readable without academic overload**: plain language first, theory terms where needed, minimal jargon stacking.
- **Action-oriented**: diagnostic and simulation text should help interpretation, not just describe mechanics.

### Content Constraints

- Prefer short, concrete sentences over abstract phrasing.
- Avoid synonym drift for core model terms.
- Keep labels stable once introduced (especially in controls and summaries).
- Distinguish clearly between choice-level metrics and problem-level metrics.

---

## Content Design Audit (Required)

Any substantial module change should include a brief content design audit.
...
- Simulation legends/captions/summaries
- Diagnosis copy and dynamic placeholders

### Audit Checks

- **Ontology check**: are all model terms used correctly?
- **Consistency check**: do labels and meanings match across all surfaces?
- **Metric-language check**: do percentages/counts describe the same unit of analysis they compute?
- **Comprehension check**: can a non-expert understand the text without losing model fidelity?
- **Tone check**: serious, clear, and grounded; no unnecessary academic density.

### Audit Output

Document findings as:

- `Issue` (what is inconsistent or unclear)
- `Impact` (how it affects interpretation)
- `Recommendation` (specific copy or labeling change)
- `Decision` (accepted/deferred and why)

Store audit notes in the relevant EPIC/PRINCIPLE doc or PR description.
