...
const overviewHtml = read('modules/satellite-index/index.html');
const demoHtml = read('modules/satellite-index/demo/index.html');
const threeHtml = read('modules/satellite-index/three/index.html');
const demoJs = read('modules/satellite-index/demo/satellite-index.js');
const threeJs = read('modules/satellite-index/three/satellite-index-three.js');
...
assertIncludes(overviewHtml, 'Load indices', 'Overview try-it copy should match Explorer action wording');
...
assertNotMatches(moduleRouteData, /cases\//, 'Legacy cases paths should stay out of module route data');
assertIncludes(moduleRouteData, "slug: 'satellite-index'", 'Boundary-free monitoring should be promoted into module route data');
assertIncludes(moduleRouteData, "title: 'Boundary-free monitoring'", 'Boundary-free monitoring module route title missing');
assertIncludes(modulesIndexHtml, 'href="satellite-index/"', 'Catalogue should link to the Boundary-free monitoring module');
assertIncludes(modulesIndexHtml, '<span class="module-demo-tag" aria-label="Demo">D</span>', 'Catalogue should mark Boundary-free monitoring with the small Demo tag');
assertIncludes(componentsCss, '.module-demo-tag', 'Demo tag styling missing from shared module card CSS');
...
assertIncludes(threeHtml, 'id="satellite-three-scene-rgb-switch" type="button" aria-pressed="false" aria-label="Toggle Sentinel scene RGB context" hidden', 'Three.js Scene RGB switch should stay off/hidden until live scene imagery exists');
assertIncludes(threeHtml, '<span class="satellite-scene-rgb-switch-label">Scene RGB off</span>', 'Three.js Scene RGB switch should expose compact state text');
...
assertIncludes(threeHtml, '<span class="satellite-terrain-switch-label">Terrain off</span>', 'Three.js terrain switch should expose the current Terrain state as compact text');
assertNotMatches(threeHtml, /satellite-terrain-switch-track|satellite-terrain-switch-thumb/, 'Three.js terrain switch should not render the old dot/track marker in the HUD');
...
assertIncludes(threeHtml, 'id="satellite-three-scene-rgb-context" hidden', 'Three.js Scene RGB context note should stay hidden until Scene RGB mode is active');
assertIncludes(threeHtml, 'Scene RGB context &middot; same Sentinel-2 acquisition as the index surface; lower resolution than the MapTiler basemap.', 'Three.js Scene RGB context note should explain temporal alignment and lower spatial resolution');
...
assert(threeHtml.indexOf('id="satellite-three-scene-rgb-switch"') < threeHtml.indexOf('id="satellite-three-terrain-switch"'), 'Three.js HUD should offer Scene RGB before Terrain in the context-switch flow');
...
assertIncludes(selectorBlock(css, '.satellite-scene-rgb-switch'), 'border-left: 1px solid var(--satellite-hud-border, var(--ink-ghost));', 'Three.js Scene RGB switch should sit as a toolbar segment');
assertIncludes(selectorBlock(css, '.satellite-scene-rgb-switch[aria-pressed="true"]'), 'background: var(--ink);', 'Three.js Scene RGB switch should use the neutral active state when on');
assertIncludes(selectorBlock(css, '.satellite-scene-rgb-switch[aria-pressed="true"]'), 'color: var(--paper);', 'Three.js Scene RGB switch active state should keep readable contrast');
...
assertIncludes(threeJs, 'function decodeRgbCanvas(base64)', 'Three.js prototype should decode same-scene Sentinel RGB imagery for the Scene RGB context');
assertIncludes(threeJs, 'sceneRgbCanvas = await decodeRgbCanvas(data.image);', 'Three.js prototype should decode Worker RGB imagery without a second request');
assertIncludes(threeJs, 'const sceneRgbTexture = sceneRgbCanvas ? makeThreeTexture(sceneRgbCanvas) : null;', 'Three.js prototype should convert the Sentinel RGB canvas into a cached base texture');
...
assertIncludes(threeJs, 'const nextTexture = lastBaseTextures[requestedMode];', 'Three.js base context switch should use cached basemap, Scene RGB, or contour textures without rerunning analysis');
...
assertIncludes(threeHtml, 'View on terrain &uarr;', 'Terrain renderer index notes should link back to the surface');
assertNotMatches(threeHtml, /satellite-three-index-tabs|satellite-three-index-tab|aria-pressed="false"[^>]*data-index/, 'Terrain renderer should not use the old index button strip');
...
assertIncludes(threeJs, 'function updateAnalysisButtonLabel(liveAllowed)', 'Three.js prototype should centralize primary button label state');
assertIncludes(threeJs, "btnEl.textContent = liveAllowed === false ? 'Zoom in' : 'Load indices';", 'Three.js primary button should switch to Zoom in when the viewport cannot load indices');
assertIncludes(threeJs, 'updateAnalysisButtonLabel(liveAllowed);', 'Three.js viewport readout should update the primary button label as the map moves');
assertIncludes(threeJs, 'const hectares = metrics.areaKm2 * 100;', 'Three.js viewport area readout should be based on hectares');
assertNotMatches(threeJs, /' km²'|' m²'/, 'Three.js viewport area readout should not mix km² or m² with the 200 ha limit');
assertIncludes(threeJs, "const sceneMetaWrap = sceneEl.closest('.satellite-three-scene-meta');", 'Three.js scene metadata should be shown only after analysis metadata is available');
assertIncludes(threeJs, 'if (sceneMetaWrap) sceneMetaWrap.hidden = false;', 'Three.js scene metadata should be unhidden after analysis');
assertIncludes(threeJs, "const sceneMetaWrap = document.querySelector('.satellite-three-scene-meta');", 'Three.js reset should find the below-viewer scene metadata wrapper');
assertIncludes(threeJs, 'if (sceneMetaWrap) sceneMetaWrap.hidden = true;', 'Three.js reset should hide scene metadata when returning to map selection');
assertIncludes(threeJs, "btnEl.textContent = 'New viewport';", 'Three.js primary button should switch to new-viewport semantics after rendering');
...
assertIncludes(threeJs, 'baseMesh.visible = true;', 'Three.js base plane should remain visible for Basemap, Scene RGB, and Terrain contexts');
assertIncludes(threeJs, "let lastBaseTextures = { satellite: null, sentinel: null, terrain: null };", 'Three.js satellite, Scene RGB, and contour base texture caches should be preserved after analysis');
assertIncludes(threeJs, "baseContextMode = requestedMode;", 'Three.js context switches should change base context without rerunning analysis');
assertIncludes(threeJs, "sceneRgbSwitchEl = document.getElementById('satellite-three-scene-rgb-switch');", 'Three.js Scene RGB switch should be wired');
assertIncludes(threeJs, "terrainSwitchEl = document.getElementById('satellite-three-terrain-switch');", 'Three.js terrain switch should be wired');
assertIncludes(threeJs, "sceneRgbContextEl = document.getElementById('satellite-three-scene-rgb-context');", 'Three.js Scene RGB context note should be wired with Scene RGB switch state');
...
assertIncludes(threeJs, "const nextMode = sceneRgbSwitchEl.getAttribute('aria-pressed') === 'true' ? 'satellite' : 'sentinel';", 'Three.js Scene RGB switch should map off to Basemap and on to same-scene Sentinel RGB');
...
assertIncludes(threeJs, "if (sceneRgbLabel) sceneRgbLabel.textContent = sceneRgbActive ? 'Scene RGB on' : 'Scene RGB off';", 'Three.js Scene RGB switch should sync visible text with the active Scene RGB state');
assertIncludes(threeJs, "if (terrainLabel) terrainLabel.textContent = terrainActive ? 'Terrain on' : 'Terrain off';", 'Three.js terrain switch should sync visible text with the active Terrain state');
assertIncludes(threeJs, 'if (sceneRgbContextEl) sceneRgbContextEl.hidden = !sceneRgbActive || !lastBaseTextures.sentinel;', 'Three.js Scene RGB context note should only show when Scene RGB mode has a live texture');
...
assertIncludes(demoJs, 'const hectares = metrics.areaKm2 * 100;', 'Viewport area readout should be based on hectares');
assertNotMatches(demoJs, /' km²'|' m²'/, 'Viewport area readout should not mix km² or m² with the 200 ha limit');
...
assertIncludes(selectorBlock(css, '.satellite-toggle-control'), 'background: var(--control-attention-surface);', 'Satellite base toggle should use the high-attention control accent');
...
assertIncludes(selectorBlock(css, '.satellite-analyse-btn'), 'background: var(--ink);', 'Analyze button should use primary-action surface rather than the high-attention control accent');
assertIncludes(css, 'box-shadow: 0.32rem 0.32rem 0 var(--control-attention-shadow);', 'Attention control option should inherit the offset-shadow switch language');
...
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status'), 'text-transform: none;', 'Three.js HUD status text should avoid alert-like all-caps styling');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status'), 'text-shadow: none;', 'Three.js HUD status text should not render a pale halo over glass pills');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status:empty'), 'display: none;', 'Three.js HUD should not render an empty status pill');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status--loading'), 'width: 0.82rem;', 'Three.js loading status should render as a quiet compact spinner');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status--loading'), 'border-top-color: var(--ink-mid);', 'Three.js loading spinner should use quiet ink contrast instead of attention color');
...
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:disabled'), 'border: 1px solid var(--satellite-hud-border);', 'Three.js zoom-needed button should use the glass primary state instead of the attention-control accent');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:disabled'), 'background: var(--satellite-hud-surface-strong);', 'Three.js zoom-needed button should keep the same blurred action surface');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:disabled'), 'backdrop-filter: var(--satellite-hud-blur);', 'Three.js zoom-needed button should own its blur surface');
assertIncludes(css, '.satellite-three-hud .satellite-toggle-control[aria-pressed="true"],\n.satellite-three-hud .satellite-scene-rgb-switch[aria-pressed="true"],\n.satellite-three-hud .satellite-terrain-switch[aria-pressed="true"] {\n  background: transparent;', 'Three.js active context toggles should remain inside the shared strip instead of becoming second pills');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'box-shadow: none;', 'Three.js HUD toolbar should stay quiet without a floating shadow');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'border: 0;', 'Three.js HUD overlay backing should avoid a rigid toolbar border');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'align-items: center;', 'Three.js HUD controls should align in one compact row');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'gap: 0.64rem;', 'Three.js HUD should separate individually blurred controls');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'background: transparent;', 'Three.js HUD toolbar should not add a shared rectangular underlay');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-controls'), 'backdrop-filter: none;', 'Three.js HUD toolbar should not blur behind all controls as one slab');
assertIncludes(css, '.satellite-three-hud .satellite-analyse-btn,\n.satellite-three-hud .satellite-toggle-control,\n.satellite-three-hud .satellite-scene-rgb-switch,\n.satellite-three-hud .satellite-terrain-switch {', 'Three.js HUD controls should share a glass-pill selector');
assertIncludes(css, '  min-height: 1.65rem;\n  padding: 0.12rem 0;\n  border: 0;\n  background: transparent;\n  -webkit-backdrop-filter: none;\n  backdrop-filter: none;', 'Three.js HUD controls should be transparent inner items inside the shared strip');
assertIncludes(css, 'text-shadow: none;', 'Three.js HUD controls should remove text halo before compacting the terrain switch');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-scene-rgb-switch-label'), 'font-size: 0.58rem;', 'Three.js Scene RGB switch label should stay compact but legible in the HUD');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-terrain-switch-label'), 'font-size: 0.58rem;', 'Three.js terrain switch label should stay compact but legible in the HUD');
assertNotMatches(css, /\.satellite-three-hud \.satellite-terrain-switch-track|\.satellite-three-hud \.satellite-terrain-switch-thumb/, 'Three.js terrain switch should use text-state chip styling instead of a dot/track marker');
assertIncludes(css, '.satellite-three-hud .satellite-scene-rgb-switch {\n  flex-direction: row;', 'Three.js Scene RGB chip should remain a horizontal text-state control');
assertIncludes(css, '.satellite-three-hud .satellite-terrain-switch {\n  flex-direction: row;', 'Three.js terrain chip should remain a horizontal text-state control');
assertIncludes(css, '  min-width: 0;\n  padding-inline: 0;\n  opacity: 0.82;\n}', 'Three.js terrain chip should stay compact in the horizontal HUD row');
assertIncludes(css, '.satellite-three-hud .satellite-scene-rgb-switch[aria-pressed="true"],\n.satellite-three-hud .satellite-terrain-switch[aria-pressed="true"] {\n  background: transparent;\n  color: var(--satellite-hud-ink);\n  border-color: transparent;\n  opacity: 1;', 'Three.js active Scene RGB chip should signal the on state without adding a dot marker');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-terrain-switch[aria-pressed="true"]'), 'opacity: 1;', 'Three.js active terrain chip should signal the on state without adding a dot marker');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'border: 1px solid var(--satellite-hud-border);', 'Three.js enabled action should be the only bordered control inside the overlay');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'border-radius: 0.38rem;', 'Three.js enabled action should render as a rounded rectangle');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'background: var(--satellite-hud-surface-strong);', 'Three.js enabled action should be the only emphasized control inside the overlay');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'backdrop-filter: var(--satellite-hud-blur);', 'Three.js enabled action should own its blur surface');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'padding-inline: 0.72rem;', 'Three.js enabled action should have breathing room inside its border');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn::before'), 'content: none;', 'Three.js HUD primary action should not render a square viewport icon cue');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout'), 'border: 0;', 'Three.js viewport readout should sit as plain metadata inside the overlay');
assertIncludes(selectorBlock(css, '.satellite-scene-rgb-switch:hover:not(:disabled)'), 'background: var(--satellite-hud-muted-surface, color-mix(in srgb, var(--ink) 8%, transparent 92%));', 'Three.js Scene RGB switch should show a visible neutral hover cue');
...
assertIncludes(css, '.satellite-three-hud .satellite-scene-rgb-switch-label {\n    display: inline-flex;', 'Three.js mobile HUD should keep the compact Scene RGB state label visible');
assertIncludes(css, '.satellite-three-hud .satellite-terrain-switch-label {\n    display: inline-flex;', 'Three.js mobile HUD should keep the compact terrain state label visible');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout'), 'background: var(--satellite-hud-surface-strong);', 'Three.js viewport readout should own a matching blurred text surface');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout'), 'backdrop-filter: var(--satellite-hud-blur);', 'Three.js viewport readout should use the same blur as the action button');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout'), 'text-transform: none;', 'Three.js viewport readout should preserve lowercase ha units');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout--blocked'), 'background: var(--satellite-hud-surface-strong);', 'Three.js blocked viewport readout should keep the same blurred text surface because the text already carries the state change');
