...
assertIncludes(selectorBlock(css, '.satellite-base-mode-switch'), 'background: var(--control-attention-surface);', 'Three.js base context switch should use the high-attention control surface token');
assertIncludes(selectorBlock(css, '.satellite-base-mode-option--active'), 'background: var(--control-attention-active-surface);', 'Three.js active base context option should use the high-attention active token');
...
assertIncludes(threeJs, "btnEl.textContent = liveAllowed === false ? 'Zoom in' : 'Analyse viewport →';", 'Three.js primary button should switch to Zoom in when the viewport cannot be analysed');
...
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status--loading'), 'border-top-color: var(--control-attention-surface);', 'Three.js loading status should render as a compact spinner');
...
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-status--loading'), 'background: color-mix(in srgb, var(--control-attention-surface) 16%, transparent 84%);', 'Three.js loading spinner should remain visible on map imagery');
...
assertIncludes(css, '.satellite-three-hud .satellite-analyse-btn:disabled {\n  border-color: color-mix(in srgb, var(--ink-ghost) 72%, transparent 28%);\n  background: color-mix(in srgb, var(--ink-ghost) 42%, transparent 58%);', 'Three.js zoom-needed button should use the disabled primary-action state instead of the attention-control accent');
assertIncludes(css, '.satellite-three-hud .satellite-toggle-control[aria-pressed="true"] {\n  border-color: var(--control-attention-surface);\n  background: var(--control-attention-surface);', 'Three.js active HUD toggles should use the high-attention control accent');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-analyse-btn:not(:disabled)'), 'background: var(--ink);', 'Three.js enabled analyze button should use primary-action ink surface');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout'), 'background: color-mix(in srgb, var(--control-attention-surface) 42%, transparent 58%);', 'Three.js blocked viewport readout should match the attention-control context accent');
assertIncludes(selectorBlock(css, '.satellite-three-hud .satellite-viewport-readout:not(.satellite-viewport-readout--blocked)'), 'background: var(--control-attention-surface);', 'Three.js viewport readout should use the attention-control accent for live context');
